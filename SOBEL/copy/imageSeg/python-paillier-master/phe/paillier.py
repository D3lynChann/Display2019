...
def generate_paillier_keypair(private_keyring=None, n_length=DEFAULT_KEYSIZE):
    ...
    return public_key, private_key#, p, q, n, n_len
    
def generate_paillier_keypair_1(n_length, li):
    """Return a new :class:`PaillierPublicKey` and :class:`PaillierPrivateKey`.
    ...
    public_key = PaillierPublicKey(n)
    private_key = PaillierPrivateKey(public_key, p, q)
    li.append(n); li.append(p); li.append(q)
    
    return public_key, private_key  
    
def generate_paillier_keypair_2(li):
    """Return a new :class:`PaillierPublicKey` and :class:`PaillierPrivateKey`.
    ...
    public_key = PaillierPublicKey(li[0])
    private_key = PaillierPrivateKey(public_key, li[1], li[2])
...
class PaillierPublicKey(object):
    ...
    def getPara(self):
        return self.g, self.nsquare, self.max_int
