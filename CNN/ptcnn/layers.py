...
def conv_forward_naive(x, w, b, conv_param):
    ...
    N, C, H, W = x.shape
    F, C, HH, WW = w.shape
    # 利用numpy.pad(array, pad_width, mode, **kwargs)，x是四维，因此要四个tube
...
def affine_forward(x, w, b):   
    ...
    # 将x变更为一行
    N = x.shape[0]
    # (N,D) -1表示不知道多少列，指定行，就能算出列 = 2 * 32 * 16 * 16/2 = 8192
    x_row = x.reshape(N, -1)         
    # (N,M) 2*8192 8192*100 =>2 * 100
        ...
        for j in range(C):        
            for k in range(H_new):            
                for l in range(W_new):
                    ...
                    # 注意到window == m：[1, 2, 3] == 3 => [False, False, True]
